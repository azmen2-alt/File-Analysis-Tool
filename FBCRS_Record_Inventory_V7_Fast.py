...
# Hide harmless background warnings
...
# BULLETPROOF PATH LOCATOR
def get_library_path():
    paths_to_check = []
    if getattr(sys, 'frozen', False):
        paths_to_check.append(os.path.join(os.path.dirname(sys.executable), 'FBCRS_Master_Full.xlsx'))
    paths_to_check.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'FBCRS_Master_Full.xlsx'))
    paths_to_check.append(os.path.join(os.getcwd(), 'FBCRS_Master_Full.xlsx'))
...
if not os.path.exists(LIBRARY):
    print("\n[CRITICAL ERROR] Rulebook Not Found!")
    print("The application could not find 'FBCRS_Master_Full.xlsx'.")
    print(f"Please ensure it is placed EXACTLY in this folder:\n{os.path.dirname(LIBRARY)}")
    input("\nPress Enter to exit...")
    sys.exit(0)
...
def read_text(fp):
    ...
    try:
        if ext == '.txt':
            text_content = open(fp, encoding='utf-8', errors='ignore').read(5000)
        elif ext == '.pdf':
            if pypdf is None:
                return ''
            ...
            text_content = ' '.join([p.text for p in doc.paragraphs[:50]])
...
def classify(fp):
    ...
    if ext not in EXTS:
        return [fn, ext, category, 'File', created, modified, '', '', '', '', '', '', '', '', '', 'REVIEW', 'UNSUPPORTED FILE TYPE', 'NO', 'NO', fp, '']

    if file_size > MAX_SIZE_BYTES:
        return [fn, ext, category, 'File', created, modified, '', '', '', '', '', '', '', '', '', 'REVIEW', 'FILE EXCEEDS CONTENT EXTRACTION LIMIT', 'NO', 'NO', fp, '']
...
def main():
    ...
    print("\n[ENGINE MODE: FAST BUT LESS ACCURATE]")
    print("* This version reads only the first few pages or rows of files.")
    print("* It is optimized to scan large data drops drastically faster.")
    ...
    while True:
        ...
        print("      [!] NOTE: It is recommended to divide files into smaller portions.\n")
        ...
        OUTPUT = os.path.join(os.path.dirname(LIBRARY), f"{folder_name} summary.xlsx")
